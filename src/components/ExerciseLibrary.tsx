...
import { Heart, Check, Search, Info, AlertTriangle, ChevronDown, ChevronUp, ArrowLeft, ArrowRight } from 'lucide-react';
...
export const ExerciseLibrary: React.FC<ExerciseLibraryProps> = ({
  ...
  const [mobileNav, setMobileNav] = useState<{
    level: 'categories' | 'category' | 'details';
  ...
  // Intercept back button gestures on mobile
  useEffect(() => {
    const handlePopState = (e: PopStateEvent) => {
      if (e.state && e.state.app === 'fitai' && e.state.view === 'library') {
        setMobileNav({
          level: e.state.level,
          category: e.state.category,
          exerciseId: e.state.exerciseId
        });
        if (e.state.category) {
          setActiveCategory(e.state.category);
        }
      } else {
        setMobileNav({ level: 'categories', category: null, exerciseId: null });
      }
    };

    window.addEventListener('popstate', handlePopState);

    // Initialize root history state if we are visiting library
    if (!window.history.state || window.history.state.view !== 'library') {
      window.history.pushState({ app: 'fitai', view: 'library', level: 'categories', category: null, exerciseId: null }, '');
    }
  ...
  const navigateToCategory = (cat: CategoryType) => {
    const nextNav = { level: 'category' as const, category: cat, exerciseId: null };
    setMobileNav(nextNav);
    setActiveCategory(cat);
    window.history.pushState({ app: 'fitai', view: 'library', ...nextNav }, '');
  };

  const navigateToDetails = (id: string) => {
    const nextNav = { level: 'details' as const, category: mobileNav.category, exerciseId: id };
    setMobileNav(nextNav);
    window.history.pushState({ app: 'fitai', view: 'library', ...nextNav }, '');
  };
  ...
  const filteredExercises = EXERCISE_DATABASE.filter((ex) => {
    const matchesCategory = ex.group === (mobileNav.level !== 'categories' && mobileNav.category ? mobileNav.category : activeCategory);
    ...
    <section id="library" className="relative py-24 overflow-hidden border-t border-white/5 bg-[#03000a]">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="md:hidden space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                {categories.map((cat) => (
                  <button
                    ...
                    className="p-6 rounded-2xl bg-dark-900/60 border border-white/5 hover:border-brand-violet/40 hover:bg-[#0c061d]/40 text-left transition-all duration-300 group shadow-glass"
                  >
                    <div className="text-white font-display font-black text-sm uppercase tracking-wider mb-2">
                      {cat}
                    </div>
                    <div className="text-[10px] text-zinc-500 font-bold uppercase tracking-wider group-hover:text-brand-cyan transition-colors">
                      Explore Exercises →
                    </div>
                  </button>
                ))}
          ...
          {mobileNav.level === 'category' && mobileNav.category && (
            <div className="space-y-6 text-left">
              {/* Prominent mobile back button */}
              <div className="flex items-center">
                <button
                  onClick={() => window.history.back()}
                  className="inline-flex items-center gap-2 text-zinc-400 hover:text-white font-black text-xs uppercase tracking-wider transition-colors min-h-[44px] min-w-[44px] py-3.5 px-4 bg-dark-900/80 border border-white/5 rounded-xl hover:bg-dark-900 shadow-glass"
                >
                  <ArrowLeft className="h-4.5 w-4.5 text-brand-cyan" /> Back to Exercise Library
                </button>
              </div>

              <div className="space-y-1">
                <span className="text-[10px] text-brand-violet font-black uppercase tracking-widest bg-brand-violet/10 px-2.5 py-0.5 rounded">
                  Category
                </span>
                <h3 className="text-2xl font-display font-black text-white">
                  {mobileNav.category} Exercises
                </h3>
              </div>
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder={`Search ${mobileNav.category.toLowerCase()}...`}
                  value={searchQuery}
              ...
              <div className="space-y-4">
                {filteredExercises.map((ex) => (
                  <div
                    key={ex.id}
                    onClick={() => navigateToDetails(ex.id)}
                    className="p-5 border border-white/5 rounded-2xl bg-dark-900/30 hover:border-brand-violet/40 cursor-pointer transition-all flex items-center justify-between shadow-glass"
                  >
                    <div className="space-y-1">
                    ...
                    <div className="p-2.5 bg-white/5 border border-white/5 rounded-xl text-zinc-500 hover:text-white">
                ...
                {filteredExercises.length === 0 && (
                  <div className="py-12 border border-dashed border-white/10 rounded-2xl text-center text-xs text-zinc-500 bg-dark-900/10">
                    No exercises found.
                  </div>
                )}
              </div>
          ...
          {mobileNav.level === 'details' && mobileNav.exerciseId && (
            <div className="space-y-6 text-left">
              {(() => {
                const ex = EXERCISE_DATABASE.find(e => e.id === mobileNav.exerciseId);
                if (!ex) return null;
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="flex items-center">
                      <button
                        ...
                        className="inline-flex items-center gap-2 text-zinc-400 hover:text-white font-black text-xs uppercase tracking-wider transition-colors min-h-[44px] min-w-[44px] py-3.5 px-4 bg-dark-900/80 border border-white/5 rounded-xl hover:bg-dark-900 shadow-glass"
                      >
                        <ArrowLeft className="h-4.5 w-4.5 text-brand-cyan" /> Back to {mobileNav.category} Exercises
                      </button>
                    </div>
                    ...
                    {/* Target specs */}
                    <div className="grid grid-cols-2 gap-4 bg-dark-900/40 p-4 border border-white/5 rounded-2xl">
                      <div>
                    ...
                    <div className="space-y-3">
                      ...
                      <ol className="space-y-2 pl-4 list-decimal text-zinc-400 font-normal leading-relaxed text-xs">
                        {ex.form.map((f, i) => (
                          <li key={i}>{f}</li>
                        ))}
                      </ol>
                    ...
                    {ex.mistakes && ex.mistakes.length > 0 && (
                      <div className="space-y-3">
                        ...
                        <ul className="space-y-2 pl-4 list-disc text-zinc-400 font-normal leading-relaxed text-xs">
                          {ex.mistakes.map((m, i) => (
                            <li key={i}>{m}</li>
                          ))}
                        </ul>
        ...
        <div className="hidden md:block space-y-12">
          ...
          <div className="space-y-6 max-w-6xl mx-auto">
            ...
            <div className="flex justify-center overflow-x-auto pb-3 gap-2 px-1 max-w-full scrollbar-thin">
              {categories.map((cat) => (
                <button
                  key={cat}
                  onClick={() => {
